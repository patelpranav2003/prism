...
2. THE Index_Builder SHALL extract the following fields from manifest.json for each dbt_Model: model name, database, schema, `alias` (the actual materialized table name in Databricks — used as the table component of the FQN; falls back to model name when absent), full qualified name constructed as `catalog.schema.alias`, all column names with descriptions, grain (from `meta.grain` if present), full compiled SQL, `depends_on.nodes` for direct parent models, tags, and folder path.
...
9. WHEN the Index_Builder processes manifest.json, THE ManifestParser SHALL additionally extract FK→PK join hints from all `test.*` nodes where `test_metadata.name == "relationships"`, deriving: the FK model name (from `kwargs.model`, unwrapping any `ref('name')` Jinja expression), the FK column (`kwargs.column_name`), the PK model name (`kwargs.to`), and the PK column (`kwargs.field`). The resulting `list[JoinHint]` SHALL be stored in the `SchemaIndex.join_hints` field. IF manifest.json contains no relationship tests, the list SHALL be empty; this condition SHALL NOT be treated as an error and SHALL NOT affect index construction.
...
4. WHEN the Retriever receives a question embedding, THE Retriever SHALL compute the cosine similarity between the question embedding and every row in the model embeddings array, then return the top-N models ordered by descending adjusted score, where N defaults to 8 (configurable per-call) and is capped at the total number of models in the index. After retrieval, the API layer SHALL additionally expand the result set with any parent models referenced in each retrieved model's `depends_on` list that are not already in the result set, ensuring dimension and lookup tables are available for JOIN generation.
...
1. WHEN the top candidate models are retrieved by the Retriever, THE Prompt_Builder SHALL construct a Claude API system prompt that includes: full column list with types and descriptions, grain, layer, and first 1500 characters of compiled SQL for each retrieved model (for prompt context; full SQL stored in schema index and shown in Schema Explorer); lineage relationships between those models from the Schema_Index adjacency list; a Join Keys section with explicit FK→PK column pairs when dbt relationship tests exist for the selected models (fallback: shared `_id`/`_key` columns appearing in exactly two selected models); and Databricks SQL dialect rules.
2. THE Prompt_Builder SHALL include the following Databricks SQL dialect rules in every system prompt: always use fully qualified names in the format `catalog.schema.table`; use backticks for column names containing special characters; use `DATE_TRUNC`, `DATEADD`, and `DATEDIFF` for date operations; use `QUALIFY` for window function filtering; apply a default `LIMIT 1000` unless the user explicitly requests more rows; never use `SELECT *`; every query must apply ALL conditions stated in the user's question — no condition may be dropped in intermediate or peek queries; match the temporal granularity of the question to the model's grain — if the question asks for daily data, use a daily-grain model rather than a weekly-grain model; for precise/specific questions, return only what was explicitly asked — do not auto-enrich with additional JOINs or context columns.
3. WHEN a model's grain field is "unknown" or the compiled SQL contains no GROUP BY or DISTINCT and the model name does not contain a `_by_` suffix, THE Prompt_Builder SHALL include an explicit deduplication instruction for that model. For row-level (non-aggregate) SELECT queries, Claude SHALL apply DISTINCT or ROW_NUMBER() logic. For aggregate queries (those using SUM, COUNT, AVG, MIN, MAX, or similar), Claude SHALL NOT apply ROW_NUMBER() deduplication — incorrect partitioning silently removes legitimate rows and produces wrong totals.
4. THE SQL_Generator SHALL call the Claude API using model `claude-sonnet-4-6` with a maximum of 4096 output tokens and SHALL request a response in the following strict JSON format: `{"sql": "...", "explanation": "...", "models_used": [...], "confidence": "high|medium|low", "confidence_reason": "..."}`. The 4096-token limit ensures complex multi-join queries with verbose explanations are never silently truncated.
...
2. THE Query_Runner SHALL authenticate to the Databricks SQL warehouse using the Workspace_OAuth_Token provided by the Databricks Apps runtime in production. In local development, IF the `DATABRICKS_TOKEN` environment variable is set, THE Query_Runner SHALL use it as a Personal Access Token — no user credential input is required or accepted at runtime.
...
6. IF the SQL execution returns an error response from the Databricks SQL warehouse, THEN THE Query_Runner SHALL automatically attempt up to two retries using the SQL_Generator: (a) **Retry 1** — classify the Databricks error into one of the following types: `column_not_found`, `table_not_found`, `type_mismatch`, `ambiguous_column`, `syntax_error`, `division_by_zero`, or `generic`; inject a targeted fix instruction for that error type into the retry prompt alongside the original question, the failed SQL, and the full error message; call the SQL_Generator with this context. (b) **Retry 2** — if Retry 1 produces SQL that also fails, send both error messages and both failed SQL strings in a single prompt with the second error's fix hint, and request a corrected query from scratch. The DDL/DML guard SHALL be applied to every SQL string produced by a retry before it is executed.
7. IF both retry executions fail, THEN THE Prism UI SHALL display: a non-technical summary message (e.g., "The query could not be completed"), the failed SQL statement in a copyable code block, and a prompt directing the user to contact a data engineer — raw Databricks error stack traces SHALL NOT be shown.
8. IF the SQL_Generator is unavailable or returns a generation failure during any retry attempt, THEN THE Query_Runner SHALL skip that retry, surface the original warehouse error and failed SQL as described in criterion 7, and log the SQL_Generator unavailability.
...
1. THE Prism UI SHALL display a landing page with the Prism gradient brand logo (triangle SVG with gradient fill) and the product name "Prism" visible in the header, a white background, and a single centered text input with placeholder text "Ask anything about your data...".
2. THE Prism UI SHALL display between 4 and 6 dynamically generated example questions derived from the current gold models in the Schema_Index, grouped by domain prefix, randomly selected per page load and fixed for the session duration, as clickable chips below the main input on the landing page; WHEN a chip is clicked, THE Prism UI SHALL populate the input with that question text and submit it automatically without requiring a separate action from the user.
...
1. THE Prism UI SHALL provide a collapsible sidebar Schema_Explorer that is hidden by default on viewports with width below 768px and visible by default on viewports with width of 768px or greater; the user SHALL be able to toggle visibility on any viewport width. The Schema_Explorer sidebar and model detail panel SHALL each be horizontally resizable by dragging a handle, with the sidebar constrained between 220px and 640px, and the detail panel between 300px and 860px.
...
1. THE Prism UI SHALL provide a Settings page at the URL `/admin` (not `/settings`, which is reserved by the Databricks Apps platform) that renders only a password prompt until the Admin_User enters the correct admin password; no settings controls SHALL be rendered or accessible in the DOM until authentication succeeds. The app SHALL always open the Chat page (`/`) on first load — the Settings page is never shown automatically and is entirely optional to use.
...
7. WHERE the Admin_User is authenticated, THE Settings page SHALL provide an App Identity section allowing the Admin_User to set and save the following optional fields: Owner Name, Owner Title, Owner Email, Team Name, and Company Name. These values SHALL be persisted to `prism_settings.json` on the server and SHALL take priority over any equivalent environment variables. WHEN at least one identity field is set, THE Prism UI SHALL display the identity information as a subtle text line at the bottom of the chat page in the order: Name · Title · Email · Team · Company.
...
1. THE Artifact_Fetcher SHALL retrieve the `GITLAB_TOKEN` exclusively from the Databricks secret scope at runtime — the token SHALL NOT appear in source code, configuration files, or environment variable default values. THE `ANTHROPIC_API_KEY` (or `OPENROUTER_API_KEY` if used as the LLM provider) and `ADMIN_PASSWORD_HASH` SHALL likewise be retrieved exclusively from the Databricks secret scope; at least one of `ANTHROPIC_API_KEY` or `OPENROUTER_API_KEY` MUST be set — Anthropic takes priority if both are present. None of these secrets SHALL appear in `app.yaml`, `docker-compose.yml`, any environment variable default value, or any log output.
...
3. THE Prism application SHALL mask secret values in all log output: for `GITLAB_TOKEN`, `ANTHROPIC_API_KEY`, and `OPENROUTER_API_KEY`, only the last 4 characters SHALL appear with all preceding characters replaced by asterisks; for `DATABRICKS_SQL_WAREHOUSE`, the value SHALL be fully masked and SHALL NOT appear in any log output in any form.
